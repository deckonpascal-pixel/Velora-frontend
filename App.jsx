...
const PAYMENT_INFO = {
  ...
  whatsapp: "2347087563363", // international format for wa.me links
};

function CartDrawer({ open, onClose, cart, products, setQty, removeItem, showToast, user, authToken }) {
  const [guestEmail, setGuestEmail] = useState("");
  const [busy, setBusy] = useState(false);
  const [placedOrder, setPlacedOrder] = useState(null); // set once the order is created, shows payment instructions
...
export default function VeloraApp() {
  ...
  const [auth, setAuth] = useState(loadStoredAuth); // { user, token } | null
  ...
  // Fetch live products from the backend; keep the built-in list as a fallback if it's unreachable.
  React.useEffect(() => {
    api("/products")
      .then((data) => setProducts(data.map(normalizeProduct)))
      .catch(() => { /* backend not running yet — keep the fallback products */ });
  }, []);

  // Handle the redirect back from Paystack (?reference=...) and verify the payment.
  React.useEffect(() => {
